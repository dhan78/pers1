...
##########################################################################################################################################################
# set path
fpath = r'C:\Users\v032823\PycharmProjects\NLP_forecast'
    ...
    for index, item in enumerate(list2):
        if item in list1:
            if index == 0:
                ...
                if list2[index - 1] not in negation_list and list2[index - 2] not in negation_list and list2[
                            index - 3] not in negation_list:
                    wordFreqDictObj[item] = wordFreqDictObj.get(item, 0) + 1
            else:
                print('error')
    return wordFreqDictObj
...
outDfObj = pd.DataFrame(columns=(
'yr', 'qtr', 'ecDt', 'stockId', 'totWords', 'totWordsMasterList', 'posWordsCnt', 'negWordsCnt', 'litWordsCnt',
'fogIdx'))
...
while yrItr < len(yrVec):
    ...
    while qtrItr < len(qtrVec):
        ...
        fn = str('C:\\Users\\fzhao\\Desktop\\NLP\\rawData\\transcriptsAll\\sp9_q' + str(qtrVec[qtrItr]) + '_' + str(
            yrVec[yrItr]) + '.txt')  # input file
        ...
        if os.path.isfile(fn) and os.stat(fn).st_size != 0:
            ...
            while sItr < 3:  # len(uniqueStockIdVec):
                ...
                while dItr < len(uniqueDtVec):
                    ...
                    #########################################################################################################################
                    outDfObj.loc[len(outDfObj)] = [yrVec[yrItr], qtrVec[qtrItr], ecDt, thisStockId, len(tokensListObj),
                                                   masterCnt, posCnt, negCnt, litCnt, fogIdx]
